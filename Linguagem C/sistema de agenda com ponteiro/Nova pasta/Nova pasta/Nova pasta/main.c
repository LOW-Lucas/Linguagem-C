...
/* run this program using the console pauser or add your own getch, system("pause") or input loop */
	...
	void excluir(Agenda **a,int *pi){
		...
		fflush(stdin);
		gets(nome);
		for(p=0;p<*pi && strncmp((*a)[p].nome,nome,strlen(nome))!=0;p++);
		if(p>=*pi){
	...
	void pesquisar(Agenda *a, int i){
		...
		if(p>=i){
			...
			printf("\n\tEmail: %s",a[p].email);
			printf("\n\tTelefone: %d",a[p].telefone);
			printf("\n\tSe deseja prosseguir, pressione 'S': ");
			o = getch();
			p++;
			if(p>i){
				p=0;
